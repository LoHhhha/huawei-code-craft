...
3. 读入并初始化泊位数据
4. 读入船的容量并初始化
5. 在 choose_best_berth 函数中选择 5 个最佳的泊位，并预处理每一点的 {要到达的泊位id, 所需时间} 信息

### 每帧流程

1. get_input 函数读入帧数据
2. 如果有机器人在当前帧即将到达泊位（读入的帧数据机器人位置在泊位前一格），那么前进并放下货物，随后主动搜索货物
3. 对于新生成的货物，调用 broadcast 方法广播信息，选择机器人搬运
4. 
